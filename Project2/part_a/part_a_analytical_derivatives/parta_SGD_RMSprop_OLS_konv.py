...
fig.suptitle('Stokastisk gradient Descent (OLS) med RMSprop')
...
while (i < Niterations and test > conv): #sjekker konvergens
    ...
    for j in range(m): #løkke over minibatchene
        ...
        diff = siv.RMSprop(beta,xi,yi,eta,delta,rho,Giter,M)
    ...
    tst = siv.RMSprop(beta,X,y,eta,delta,rho,Giter,n)
...
plt.xlabel(r'$x$')
plt.ylabel(r'$y$')
plt.title(r'Stokastisk gradient descent med RMSprop')
plt.legend()
plt.show()
