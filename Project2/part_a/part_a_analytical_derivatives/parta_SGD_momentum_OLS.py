...
fig.suptitle('Stokastisk gradient Descent (OLS) med momentum')
...
while (i < Niterations and test > conv): #sjekker konvergens
    ...
    for j in range(m): #løkke over minibatchene
        ...
        diff, beta_prev = siv.GDM(beta,xi,yi,eta,gamma,beta_prev,M)
    ...
    tst, beta_prev = siv.GDM(beta,X,y,eta,gamma,beta_prev,n)
...
plt.xlabel(r'$x$')
plt.ylabel(r'$y$')
plt.title(r'Stokastisk gradient descent med momentum')
plt.legend()
plt.show()
