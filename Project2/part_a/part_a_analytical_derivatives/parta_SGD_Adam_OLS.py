...
rho1 = np.logspace(-4,-1,4)
rho2 = np.logspace(-4,-1,4)
#rho1, rho2 = 0.9, 0.99 #parametere til ADAM

MSE_GD = np.ones([np.size(eta),np.size(rho1),np.size(rho2)])
R2_GD = np.zeros([np.size(eta),np.size(rho1),np.size(rho2)])
MSE_OLS = np.ones([np.size(eta),np.size(rho1),np.size(rho2)])
R2_OLS = np.zeros([np.size(eta),np.size(rho1),np.size(rho2)])
...
for ln in range(np.size(eta)): #løkke over eta-verdier
    for r1 in range(np.size(rho1)):
        for r2 in range(np.size(rho2)):
            ...
            while (i < Niterations and test > conv): #sjekker konvergens
                for epoch in range(n_epochs): #løkke over epokene
                    Giter = 0.0 #brukes i adagrad og ADAM
                    ...
                    for j in range(m): #løkke over minibatchene
                        ...
                        diff = siv.ADAM(beta,xi,yi,eta[ln],delta,rho1[r1], rho2[r2],first_moment,second_moment, Giter,epoch+1,M)
                        beta += diff
                                    
                        i =i+1
             
                    #tester konvergens med hele gradienten!
                    tst = siv.ADAM(beta,X,y,eta[ln],delta,rho1[r1], rho2[r2],first_moment,second_moment, Giter,epoch+1,n)
            ...
            MSE_GD[ln][r1][r2] = siv.MSE(ynew,ypredict)
            R2_GD[ln][r1][r2]= siv.R2(ynew,ypredict)
            MSE_OLS[ln][r1][r2] = siv.MSE(ynew,ypredict2)
            R2_OLS[ln][r1][r2]= siv.R2(ynew,ypredict2)
            ...
            plt.xlabel(r'$x$')
            plt.ylabel(r'$y$')
            plt.title(r'Stokastisk gradient descent med Adam')
            plt.legend()
            plt.show()
...
plt.figure()
sns.heatmap(data=MSE_GD[:,:,2], annot=True)
plt.title('MSE SGD med Adam')
plt.xlabel('rho1 number')
plt.ylabel('learning rate number')


plt.figure()
sns.heatmap(data=R2_GD[:,:,2], annot=True)
plt.title('R2 SGD med Adam')
plt.xlabel('rho1 number')
plt.ylabel('learning rate number')


plt.figure()
sns.heatmap(data=MSE_GD[:,1,:], annot=True)
plt.title('MSE SGD med Adam')
plt.xlabel('rho2 number')
plt.ylabel('learning rate number')


plt.figure()
sns.heatmap(data=R2_GD[:,1,:], annot=True)
plt.title('R2 SGD med Adam')
plt.xlabel('rho2 number')
plt.ylabel('learning rate number')
...
plt.figure()
sns.heatmap(data=MSE_GD[3,:,:], annot=True)
plt.title('MSE SGD med Adam')
plt.xlabel('rho2 number')
plt.ylabel('rho1 number')


plt.figure()
sns.heatmap(data=R2_GD[3,:,:], annot=True)
plt.title('R2 SGD med Adam')
plt.xlabel('rho2 number')
plt.ylabel('rho1 number')