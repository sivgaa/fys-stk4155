...
#eta = [5.e-02, 1.e-01, 5.e-01]
#eta = 1.e-01 / np.max(EigValues) #learning rate < 2/(den største egenverdien til H)
eta =0.01
...
fig.suptitle('Stokastisk gradient Descent (OLS) med Adagrad')
...
while (i < Niterations and test > conv): #sjekker konvergens
    ...
    for j in range(m): #løkke over minibatchene
        ...
        diff = siv.Adagrad(beta,xi,yi,eta,delta,Giter,M)
    ...
    tst = siv.Adagrad(beta,X,y,eta,delta,Giter,n)
