...
def cost_mse(beta,x,y):
    return np.mean((y-x @ beta)**2)

#gjør om denne til å bruke autograd
def GD_vanlig(beta, x, y,eta,n):
    #gradient = (2.0/n)*x.T @ (x @ beta-y)
    gradient = grad(cost_mse,0) #beta er 0te argument i cost_mse
    diff = -eta*gradient(beta,x,y) #vanlig GD
    return diff

def GDM(beta,x,y,eta,gamma,beta_prev,n):
    #gradient = (2.0/n)*x.T @ (x @ beta-y)
    gradient = grad(cost_mse,0) #beta er 0te argument i cost_mse
    diff = -eta*gradient(beta,x,y) + gamma*(beta - beta_prev)
...
def Adagrad(beta,x,y,eta,delta,Giter,n):
    #gradient = (2.0/n)*x.T @ (x @ beta-y)
    gradient = grad(cost_mse,0) #beta er 0te argument i cost_mse
    Giter += gradient(beta,x,y)*gradient(beta,x,y)
    diff = -gradient(beta,x,y)*eta/(np.sqrt(Giter)+delta)
    return diff

def RMSprop(beta,x,y,eta,delta,rho,Giter,n):
    #gradient = (2.0/n)*x.T @ (x @ beta-y)
    gradient = grad(cost_mse,0) #beta er 0te argument i cost_mse
    Giter = (rho*Giter+(1-rho)*gradient(beta,x,y)*gradient(beta,x,y))
    diff = -gradient(beta,x,y)*eta/(np.sqrt(Giter)+delta)
    return diff
...
def ADAM(beta,x,y,eta,delta,rho1, rho2,first_moment,second_moment, Giter,i,n):
    #gradient = (2.0/n)*x.T @ (x @ beta-y)
    gradient = grad(cost_mse,0) #beta er 0te argument i cost_mse
    first_moment = rho1*first_moment + (1-rho1)*gradient(beta,x,y)
    second_moment = rho2*second_moment+(1-rho2)*gradient(beta,x,y)*gradient(beta,x,y)
...
def FrankeFunction(x,y):
    ...
    return term1 + term2 + term3 + term4 + np.random.normal(0,1,x.shape)
